...
const TRUST_CHIPS = [
  ...
  { label: "Browser-Based", icon: "browser" },
          ...
          <div className="flex flex-wrap justify-center gap-2 pt-1">
            {TRUST_CHIPS.map((chip) =>
              chip.icon === "github" ? (
                ...
                <span key={chip.label} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-(--color-surface) border border-(--color-border) text-(--color-text-muted)">
                  <TrustIcon name={chip.icon} />
                  {chip.label}
                </span>
              )
            )}
